...
module.exports = __webpack_require__(/*! /Users/dan/code/drop-game/src/main.js */331);
...
var _Game = __webpack_require__(/*! ./states/Game */ 338);
...
var _GameOver = __webpack_require__(/*! ./states/GameOver */ 347);
...
var _GameService = __webpack_require__(/*! ../services/GameService */ 339);
...
var _class = function (_Phaser$State) {
  ...
  _createClass(_class, [{
    ...
    value: function create() {
      ...
      this.gameService = new _GameService2.default(this, this.gameOver);
    ...
    value: function update() {
      ...
      this.game.physics.arcade.overlap(this.gameService.player, this.gameService.tileService.group, this.collisionHandler.bind(this), null, this);
    }
  }, {
    key: 'collisionHandler',
    value: function collisionHandler(a, b) {
      b.kill();
      a.tween();
      this.sound.play();
...
var _TileService = __webpack_require__(/*! ./TileService */ 340);

var _TileService2 = _interopRequireDefault(_TileService);

var _SnowService = __webpack_require__(/*! ./SnowService */ 342);

var _SnowService2 = _interopRequireDefault(_SnowService);

var _EmitterService = __webpack_require__(/*! ./EmitterService */ 343);

var _EmitterService2 = _interopRequireDefault(_EmitterService);

var _UIService = __webpack_require__(/*! ./UIService */ 344);

var _UIService2 = _interopRequireDefault(_UIService);

var _Player = __webpack_require__(/*! ../sprites/Player */ 346);
...
var GameService = function () {
  function GameService(state) {
    ...
    this.snowService = new _SnowService2.default();

    this.tileService = new _TileService2.default({ game: this.game });
    this.player = new _Player2.default({ game: this.game });
    this.UIService = new _UIService2.default({ game: this.game });
    this.EmitterService = new _EmitterService2.default({ game: this.game });
  ...
  _createClass(GameService, [{
    ...
    key: 'onRelease',
    value: function onRelease(_ref3) {
    ...
    key: 'updateScore',
    value: function updateScore(score, x, y) {
    ...
    value: function render() {
      ...
      this.tileService.group.forEachAlive(this.game.debug.body, this.game.debug, '#ff9090', false);
    }
  }]);
...
var _Tile = __webpack_require__(/*! ../sprites/Tile */ 341);
...
var TileService = function () {
  function TileService() {
    ...
    for (var i = 0; i < 50; i++) {
      var tile = new _Tile2.default({ game: this.game });
  ...
  _createClass(TileService, [{
    ...
    value: function spawn() {
      var tile = this.group.getFirstDead();
      if (tile) {
...
var _class = function (_Phaser$Sprite) {
  ...
  function _class(_ref) {
    ...
    _this.scale.set(window.scaleRatio);
    _this.kill();
    var width = _this.game.width;
    if (width > 500) {
      width = 500;
    }
    var mid = _this.game.width / 2;
    _this.lanes = [mid - width / 2.2, mid, mid + width / 2.2];

    game.physics.enable(_this, _phaser2.default.Physics.ARCADE);
    var _x = _this.width / 2;
    _this.body.setSize(_x / 3, _x / 5, _x * 1.5, _x);
    return _this;
  }

  _createClass(_class, [{
    key: 'reset',
    value: function reset(lane, speed) {
      var x = this.lanes[lane];
      _get(_class.prototype.__proto__ || Object.getPrototypeOf(_class.prototype), 'reset', this).call(this, x, -200);
      this.visible = true;
...
var MAX_SNOW = 20;
var BASE_Y_SPEED = 5;
var FLAKE_BUFFER = 320 / 2;
var SNOW_BUFFER = 5;
var NUM_SNOW_SCALES = 4;
var NUM_FLAKE_SCALES = 4;

var SnowService = function () {
  function SnowService(state) {
    _classCallCheck(this, SnowService);

    this.game = window.game;

    this.snow = [];
    for (var i = 0; i < MAX_SNOW * window.devicePixelRatio; i++) {
      var graphic = this.game.add.graphics(0, 0);

      graphic.beginFill(0xffffff, 1);
      graphic.drawCircle(0, 0, 15);
      this.snow.push(graphic);
      this.updateSnow(i, Math.random() * this.game.width, Math.random() * this.game.height);
    }

    this.flakes = [];
    for (var _i = 0; _i < MAX_FLAKES * window.devicePixelRatio; _i++) {
      var flake = this.game.add.sprite(0, 0, 'flakes');
      flake.anchor.set(0.5);
      this.flakes.push(flake);
      this.updateFlake(_i, Math.random() * this.game.width, Math.random() * this.game.height);
    }
    this.angle = 0;
    this.snowAngle = 0;
  }

  _createClass(SnowService, [{
    key: 'update',
    value: function update() {
      this.angle += 0.001;
      this.snowAngle += 0.01;
      ...
      for (var i = 0; i < MAX_FLAKES * window.devicePixelRatio; i++) {
        var p = this.flakes[i];
        var buffer = FLAKE_BUFFER * window.scaleRatio;
      ...
      for (var _i2 = 0; _i2 < MAX_SNOW * window.devicePixelRatio; _i2++) {
        var _p = this.snow[_i2];
        var _buffer = SNOW_BUFFER * window.scaleRatio;
        _p.y += Math.cos(this.angle + _p.d) + BASE_Y_SPEED + _p.r / 2;
        _p.x += Math.sin(this.angle) * 2;

        if (_p.x > W + _buffer || _p.x < -_buffer || _p.y > H + _buffer) {
          if (_i2 % 3 > 0) {
            this.updateSnow(_i2, Math.random() * W, -_buffer);
          } else {
            if (Math.sin(this.angle) > 0) {
              this.updateSnow(_i2, -_buffer, Math.random() * H);
            } else {
              this.updateSnow(_i2, W + _buffer, Math.random() * H);
            }
          }
        }
      }
    ...
    key: 'updateFlake',
    value: function updateFlake(i, x, y) {
    ...
    key: 'updateSnow',
    value: function updateSnow(i, x, y) {
      var snow = this.snow[i];
      snow.x = x;
      snow.y = y;
...
var EmiterService = function () {
  function EmiterService(_ref) {
    var game = _ref.game;

    _classCallCheck(this, EmiterService);
  ...
  _createClass(EmiterService, [{
    key: 'emit',
    value: function emit(x, y) {
      var amount = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 2;
  ...
  return EmiterService;
}();

exports.default = EmiterService;
...
var _PointText = __webpack_require__(/*! ../sprites/PointText */ 345);
...
var UIService = function () {
  function UIService(_ref) {
    ...
    var width = 400 * window.scaleRatio;
    var height = 140 * window.scaleRatio;
    ...
    for (var i = 0; i < 10; i++) {
      var pointText = new _PointText2.default({ game: game });
  ...
  _createClass(UIService, [{
    key: 'updateScore',
    value: function updateScore(n, x, y) {
      this.score += n;
      var pointText = this.pointTexts.getFirstDead();
      if (pointText) {
        pointText.reset(x, y, n);
      }
      this.scoreText.text = this.score.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    }
  }, {
    key: 'update',
    value: function update() {
      this.bar.scale.x = (this.timer.tick - this.timer.timer._now) / this.timer.delay;
    }
  }, {
    key: 'endGame',
...
var _class = function () {
  ...
  _createClass(_class, [{
    ...
    value: function tween() {
      ...
      var tween = this.game.add.tween(this).to({ y: this.y - 200, alpha: 0.8 }, 500, _phaser2.default.Easing.Quadratic.Out, true);
      tween.onComplete.add(function () {
        var tween = _this.game.add.tween(_this).to({ alpha: 0 }, 1000, _phaser2.default.Easing.Quadratic.Out, true);
        tween.onComplete.add(_this.kill.bind(_this));
      });
    }
  }]);
...
var _class = function (_Phaser$Sprite) {
  ...
  function _class(_ref) {
    ...
    var _this = _possibleConstructorReturn(this, (_class.__proto__ || Object.getPrototypeOf(_class)).call(this, game, game.width / 2, game.height - 260 * window.scaleRatio, 'tile'));

    _this.group = game.add.group();
    _this.game.physics.arcade.enable(_this);
    _this.group.add(_this);
    _this.reset(_this.x, _this.y);
    _this.frame = 0;

    _this.scaleRatio = window.scaleRatio;

    _this.scale.setTo(_this.scaleRatio);
    _this.anchor.setTo(0.5, 1);
    _this.lane = 1;
    _this.game = game;

    var x = _this.game.width / 2;
    _this.lanes = [0 + x / 2.5, x, _this.game.width - x / 2.5];
    game.physics.enable(_this, _phaser2.default.Physics.ARCADE);
    var _x = _this.width / 2;

    _this.body.setSize(_x, _x / 10, _x, _x);
    return _this;
  }
  ...
  _createClass(_class, [{
    ...
    value: function tween() {
      ...
      var tween = this.game.add.tween(this.scale).to({ x: this.scaleRatio * 1.08, y: this.scaleRatio * 1.2 }, 100, _phaser2.default.Easing.Quadratic.Out, true);

      tween.onComplete.add(function () {
        var tween = _this2.game.add.tween(_this2.scale).to({ x: _this2.scaleRatio, y: _this2.scaleRatio }, 250, _phaser2.default.Easing.Quadratic.Out, true);
        tween.onComplete.add(function () {
...
var _class = function (_Phaser$State) {
  ...
  _createClass(_class, [{
    ...
    value: function create(args) {
      ...
      var image = this.game.add.image(this.game.width / 2, this.game.height / 2, 'background');
      var scale = this.game.width / image.width;
      var scaleY = this.game.height / image.height;
...
var _class = function (_Phaser$State) {
  ...
  _createClass(_class, [{
    ...
    value: function create(args) {
      ...
      this.stage.backgroundColor = '#ffffff';
      var image = this.game.add.image(this.game.width / 2, this.game.height / 2, 'menu');
      var scale = this.game.width / image.width;
      var scaleY = this.game.height / image.height;
      image.anchor.set(0.5);
      image.scale.set(scale > scaleY ? scaleY : scale);
